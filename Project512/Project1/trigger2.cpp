...
BOOL cTaskZhuXian::Process0(int stage)
{
	...
	case 0:
		
		{
			...
			g_pCharacter->SetData(&g_pMainState->m_pCharacterDataList[eJuSe杀破狼]);
			g_pCharacter->m_PCData.m_MengPaiID = eMP_五庄观;
...
//	int lv = g_pCharacter->m_PCData.m_LV;
	...
	case 12:
		
		{
			   ...
			   {
				   g_pMainState->m_Tags.Add("未配置属性点");
				   return TRUE;
			   }
	...
	case 30:
		...
		break;
	case 31:
		g_pMainState->m_TriggerDialog.AddSelect(this, "要观看帮战吗?", g_pMainState->m_pNowSelect->m_TrueName.GetString(), -1,
			"是", 32,
			"否", -1
			);
		break;
