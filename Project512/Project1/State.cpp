...
cObj* CMainState::GetPointAt(long x, long y) 
{
	for (int i = 0; i < MAX_OBJ_NUM; i++)
	{
		if (m_pObj[i])
		{
			...
			{
				if (m_pObj[i] == m_FightList[m_HeroID].m_pData)continue;//忽略主角
				return m_pObj[i];
			}
		}
	}
...
BOOL CMainState::ProcessInterface()
{
	...
	if (Keyboard.GetKeyState(DIK_H))
	{
		...
		switch (i2)
		{
		case POS_STAND:oss << "站"; break;
		case	POS_WALK:oss << "走"; break;
		case	POS_FIGHT_STAND:oss << "战"; break;
		case	POS_RUN:oss << "跑"; break;
		case	POS_ATTACK:oss << "攻"; break;
		case	POS_MAGIC:oss << "法"; break;
		case	POS_DEF:oss << "防"; break;
		case	POS_SUFFER:oss << "受"; break;
		case	POS_DEAD:oss << "死"; break;
		case	POS_RETURN:oss << "转"; break;
		case	POS_ATTACK2:oss << "攻二"; break;
		}
		path = oss.str();
		...
		/*g_pCharacter->m_PCData.modeid = eJuSe神天兵;
			...
			for (int j = 0; j < m_pList[i]->m_work.size(); j++)
			{
				...
				
					{
						if (pUserData->m_FightProperty.cannotAttack)
						{
							...
							if (i == g_pMainState->m_HeroID)
								g_pMainState->m_Channel.AddNewString("无法使用物理攻击");
							break;
						}
				...
				case WORKTYPE_ITEM:
					//使用道具
						...
						if (pUserData->m_FightProperty.cannotAll)
						{
							m_pList[i]->m_work.clear();
							if (i == g_pMainState->m_HeroID)
								g_pMainState->m_Channel.AddNewString("无法使用道具");
							break;
						}
						g_pMainState->m_SkillManager.Use(m_pList[i]->m_SkillSelect, i);
				...
				case WORKTYPE_SUMMON:
					if (pUserData->m_FightProperty.cannotAll)
					{
						m_pList[i]->m_work.clear();
						if (i == g_pMainState->m_HeroID)
							g_pMainState->m_Channel.AddNewString("无法召唤");
						break;
					}
					if (g_pMainState->m_SkillManager.Use(m_pList[i]->m_SkillSelect, i))
					{
