...
private:
	...
	//{
	//	WORD  hdw;  //头两个字节
	//	DWORD sst;  //第七个字节开始的四个字节
	//	DWORD nst;  //第九个字节开始的四个字节
	//	DWORD dss;  //倒数六个字节开始的四个字节
	//	WORD sss;	//第五个字节开始的二个字节
	//	BYTE dsg[3];//倒数三个字节
	//};
...
struct sFightData  //表示角色战斗时的属性 (战斗时属性会随得到的BUFF而改变.我们当然不能直接拿角色的永久属性来变)
{
	...
	int m_MaxDef;
	int m_MaxMagicDef;
	//记录强化状态  第0位+5%(比如魔兽之印) 第1位+10%(比如野兽) 第2位 -5% 第3位-10% (比如放下)  后面的位数可以以后拓展
	int m_AtkIntensify;
	int m_DefIntensify;
	int m_SpdIntensify;
	int m_WallNum; //
